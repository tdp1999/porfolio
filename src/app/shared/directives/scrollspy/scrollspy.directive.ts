        ...
        this._scrollService
            ...
            .subscribe((event: Event) => {
                ...
                for (let i = 0; i < children.length; i++) {
                    ...
                    const isInsideViewport = this._scrollService.isElementInsideViewport(element);
                    isInsideViewport && sectionsInView.push(element);
                }

                // Find the section that has the biggest height visible in the viewport
                const currentSection = this._scrollService.elementWithMaxVisibleHeight(sectionsInView)?.id;

                this._activeSection = currentSection;
                this.sectionChange.emit(this._activeSection ?? null);
                this._scrollService.setActiveSection(this._activeSection ?? null);
            });
    }
